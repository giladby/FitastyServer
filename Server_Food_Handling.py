...
def check_insert_dish_params(data):
    error = True
    if data and name_field_param in data and ingredients_field_param in data and \
            dishes_field_param in data and is_liquid_field_param in data and \
            len(data) == 4:
        error = False
    return error
...
def insert_dish_by_data(data):
    ...
    return insert_dish(name, is_liquid, ingredients, dishes)
...
def server_insert_dish(server):
    ...
    if not error:
        dish_name = data[name_field_param]
...
def server_get_ingredient_info(server):
    ...
    qs = parse_qs(urlparse(server.path).query)
    if name_field_mysql in qs and len(qs) == 1:
        name = qs[name_field_mysql][0]
        error = False

    if not error:
        error, found, result = get_ingredient_info_by_name(name)
